...
const DevPanel = React.memo(function DevPanel() {
  ...
  const fetchStatus = useCallback(async () => {
    try {
      ...
      errToast(`Status fetch failed: ${error.response?.status || ''} ${error.message}`);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const host = new URL(cfg.orchestrator).host;
    const proto = window.location.protocol === 'https:' ? 'wss' : 'ws';
    const wsUrl = `${proto}://${host}/ws/logs/${logSource}?api_key=${encodeURIComponent(cfg.API_KEY)}`;
    ...
    (serviceName, action) => {
      const run = debounce(async () => {
        try {
          ...
          await axios.post(
            `${cfg.orchestrator}/services/${serviceName}/${action}`,
            {},
            { headers: { 'X-API-Key': cfg.API_KEY } },
          );
          await fetchStatus();
          ...
          errToast(`Failed to ${action} ${serviceName}: ${error.response?.status || ''} ${error.message}`);
        }
      }, 60);
      run();
    },
