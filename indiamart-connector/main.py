...
INDIAMART_MIN_INTERVAL_S = int(os.getenv("INDIAMART_MIN_INTERVAL_S", "300"))  # 5 min
_last_pull_at: Optional[datetime] = None
_pull_lock = asyncio.Lock()

async def respect_min_interval():
    """Ensure at least N seconds between IndiaMART API calls across the process."""
    global _last_pull_at
    async with _pull_lock:
        now = datetime.utcnow()
        if _last_pull_at:
            delta = (now - _last_pull_at).total_seconds()
            wait = INDIAMART_MIN_INTERVAL_S - delta
            if wait > 0:
                jitter = random.uniform(0, 2)  # tiny jitter to avoid thundering herd
                w = wait + jitter
                log.info("indiamart_throttle_wait", seconds=round(w, 1))
                await asyncio.sleep(w)
        _last_pull_at = datetime.utcnow()
...
async def log_requests(request, call_next):
    ...
    print(f"[HTTP] {request.method} {request.url} body={body[:500]!r}")
...
async def fetch_indiamart_leads(start_dt: datetime, end_dt: datetime, max_attempts: int = 3) -> List[dict]:
        ...
        try:
            async with aiohttp.ClientSession(timeout=timeout) as session:
                async with session.get(url, params=params) as resp:
                    ...
                    # IndiaMART often returns 200 with CODE!=200 inside the JSON.
                    ...
                    # Success
                    if code in (200, "200") and isinstance(raw, list):
                        return raw

                    # Rate-limited → wait and retry
                    if code in (429, "429"):
                        wait = INDIAMART_MIN_INTERVAL_S + random.uniform(1, 3)
                        log.warning("indiamart_rate_limited", attempt=attempt, wait=round(wait, 1), message=message)
                        await asyncio.sleep(wait)
                        continue

                    # Other non-OK code
                    log.warning("indiamart_non_ok_code", code=code, message=message)
                    return []

        except Exception as ex:
            backoff = min(30, 2 ** attempt) + random.uniform(0, 1)
            log.error("indiamart_fetch_failed", error=str(ex), attempt=attempt, backoff=round(backoff, 1))
            await asyncio.sleep(backoff)

    log.error("indiamart_fetch_gave_up", attempts=max_attempts)
    return []
...
async def process_batch_jobs(rmq_url: str, jobs: List[PullRequest]):
    ...
    A global throttle ensures ≥ INDIAMART_MIN_INTERVAL_S between API hits.
    """
    ...
    try:
        ...
        for job in jobs:
            ...
            # Respect global cadence and fetch
            await respect_min_interval()
...
async def schedule_pull(jobs_body: Union[List[PullRequest], Dict[str, List[PullRequest]]], background_tasks: BackgroundTasks):
    ...
    # Log what we got
    print(f"[pull/batch] received {len(jobs)} jobs")
    ...
    print(f"[pull/batch] scheduled {len(jobs)} jobs")
    return {"status": "scheduled", "jobs": len(jobs)}

@app.post("/debug/echo")
async def echo(payload: dict):
    print("[echo] payload:", payload)
